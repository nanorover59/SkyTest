...
public class SphereTexture {
    ...
    private static final int UBO_SIZE = new Std140SizeCalculator().putVec3().putVec3().putVec4().get();
    ...
    private static final RenderPipeline PIXEL_GEOMETRY = RenderPipelines.register(
            RenderPipeline.builder(RenderPipelines.DEBUG_FILLED_SNIPPET)
                    ...
                    .withVertexShader(SkytestClient.modID("core/sphere"))
                    .withFragmentShader(SkytestClient.modID("core/sphere"))
    ...
    public void prepareTexture(AbstractTexture texture) {
        ...
        VertexConsumer buffer = this.stagedBuffer.getVertexBuilder(draw);
        Matrix4fc positionMatrix = matrices.last().pose();

        buffer.addVertex(positionMatrix, -1.0f, -1.0f, 0.0f).setColor(1.0f, 1.0f, 1.0f, 1.0f);
        buffer.addVertex(positionMatrix, 1.0f, -1.0f, 0.0f).setColor(1.0f, 1.0f, 1.0f, 1.0f);
        buffer.addVertex(positionMatrix, 1.0f, 1.0f, 0.0f).setColor(1.0f, 1.0f, 1.0f, 1.0f);
        buffer.addVertex(positionMatrix, -1.0f, 1.0f, 0.0f).setColor(1.0f, 1.0f, 1.0f, 1.0f);
        ...
        if(info != null) {
            ...
            try (GpuBufferSlice.MappedView view = this.ubo.currentBuffer().map(false, true)) {
                ...
                Vector4f sphere = new Vector4f(0.0f, 0.0f, 0.0f, 0.5f);
                Std140Builder.intoBuffer(view.data()).putVec3(light).putVec3(viewPos).putVec4(sphere);
            }
            ...
            try(RenderPass renderPass = device.createCommandEncoder().createRenderPass(() -> "Test", this.textureView, Optional.empty(), this.depthTextureView, OptionalDouble.of(0))) {
                ...
                renderPass.setUniform("SphereInfo", this.ubo.currentBuffer());
